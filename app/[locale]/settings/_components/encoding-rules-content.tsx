...
const mockDocumentRules = [
  {
    type: '采购订单 (Purchase Order)',
    prefix: 'PO',
    dateFormat: 'YYYYMMDD',
    seqLength: 3,
    preview: 'PO-20240520-001',
  },
  {
    type: '采购入库 (Purchase Receipt)',
    prefix: 'PI',
    dateFormat: 'YYYYMMDD',
    seqLength: 3,
    preview: 'PI-20240520-001',
  },
  {
    type: '销售订单 (Sales Order)',
    prefix: 'SO',
    dateFormat: 'YYYYMMDD',
    seqLength: 4,
    preview: 'SO-20240520-0001',
  },
  {
    type: '销售发货 (Sales Dispatch)',
    prefix: 'SD',
    dateFormat: 'YYYYMMDD',
    seqLength: 4,
    preview: 'SD-20240520-0001',
  },
]
    ...
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950">
      ...
      <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/50 px-6 py-4 dark:border-slate-800 dark:bg-slate-900/50">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-bold text-slate-800 dark:text-slate-200">{t('documentRules')}</h2>
        </div>
          ...
          <tbody className="divide-y divide-slate-50 text-sm font-medium dark:divide-slate-800/50">
            {mockDocumentRules.map((rule, idx) => (
              <tr key={idx} className="transition-colors hover:bg-slate-50/50 dark:hover:bg-slate-800/50">
                <td className="px-6 py-4 text-slate-900 dark:text-slate-100">{rule.type}</td>
                <td className="text-primary px-6 py-4 font-bold">{rule.prefix}</td>
                <td className="px-6 py-4 text-slate-500 dark:text-slate-400">{rule.dateFormat}</td>
                <td className="px-6 py-4 text-slate-500 dark:text-slate-400">{rule.seqLength}</td>
                ...
                <td className="px-6 py-4 text-right">
                  <button className="text-primary text-xs font-bold hover:underline">{commonT('edit')}</button>
                </td>
              </tr>
            ))}
          </tbody>
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950">
      ...
      <div className="flex items-center gap-2 border-b border-slate-100 bg-slate-50/50 px-6 py-4 dark:border-slate-800 dark:bg-slate-900/50">
        ...
        <h2 className="text-sm font-bold text-slate-800 dark:text-slate-200">{t('materialBatchRules')}</h2>
      </div>
...
function SmartSuggestionCard() {
  ...
  return (
    <div className="bg-primary relative flex flex-col justify-center overflow-hidden rounded-xl p-8 text-white shadow-sm">
      <div className="relative z-10">
        ...
        {/* 描述 */}
        <p className="mb-6 text-sm leading-relaxed text-slate-200">{t('smartSuggestionDesc')}</p>
        ...
        <Button
          variant="secondary"
          className="text-primary w-full gap-2 bg-white py-3 font-bold shadow-lg transition-all hover:scale-[1.02] hover:bg-white/90 active:scale-[0.98]"
        >
