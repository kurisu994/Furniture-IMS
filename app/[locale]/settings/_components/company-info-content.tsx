...
import { Building2, Clock, Coins, Contact, Globe, ImageIcon, Mail, MapPin, Pencil, Phone, Save, Settings2, User, X } from 'lucide-react'
  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
      <div className="mb-8 flex items-center justify-between">
        ...
        {!isEditing && (
          <Button variant="default" size="sm" className="gap-1.5 shadow-sm" onClick={onEditClick}>
      ...
      <div className="grid grid-cols-1 gap-x-12 gap-y-8 md:grid-cols-2">
        ...
        <div className="space-y-1.5">
          ...
          {isEditing ? (
            ...
            <div className="min-h-6 text-base font-bold text-slate-900 dark:text-slate-100">{data.companyName || '—'}</div>
          )}
        </div>
        ...
        <div className="space-y-1.5">
          ...
          {isEditing ? (
            ...
            <div className="min-h-6 font-mono text-base font-bold tracking-tight text-slate-900 dark:text-slate-100">{data.taxId || '—'}</div>
          )}
        </div>
  ...
  return (
    <div className="flex items-center justify-between rounded-lg border border-slate-100 bg-slate-50/50 p-4 dark:border-slate-800 dark:bg-slate-900/50">
      ...
      {isEditing && options && onValueChange ? (
        ...
        <span className="text-primary text-sm font-bold">{options?.find(o => o.value === value)?.label || value || '—'}</span>
      )}
    </div>
  )
}
  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
      ...
      <div className="space-y-6">
        ...
        />
        <ContactItem icon={Phone} label={t('phone')} value={data.phone} isEditing={isEditing} onValueChange={val => onChange('phone', val)} />
        <ContactItem
          icon={Mail}
          label={t('email')}
          type="email"
    ...
    async function loadData() {
      try {
        ...
        configs.forEach(record => {
          if (record.key === SystemConfigKeys.COMPANY_NAME) mergedData.companyName = record.value
          if (record.key === SystemConfigKeys.COMPANY_TAX_ID) mergedData.taxId = record.value
          if (record.key === SystemConfigKeys.COMPANY_ADDRESS) mergedData.address = record.value
          if (record.key === SystemConfigKeys.COMPANY_BUSINESS_TYPE) mergedData.businessType = record.value
          if (record.key === SystemConfigKeys.COMPANY_CONTACT_NAME) mergedData.contactName = record.value
          if (record.key === SystemConfigKeys.COMPANY_PHONE) mergedData.phone = record.value
          if (record.key === SystemConfigKeys.COMPANY_EMAIL) mergedData.email = record.value
          if (record.key === SystemConfigKeys.DEFAULT_LOCALE) mergedData.defaultLanguage = record.value
          if (record.key === SystemConfigKeys.BASE_CURRENCY) mergedData.baseCurrency = record.value
          if (record.key === SystemConfigKeys.TIMEZONE) mergedData.timezone = record.value
        })
    ...
    <div className="flex w-full flex-col gap-6">
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="flex flex-col gap-6 lg:col-span-2">
          <BasicInfoCard data={data} isEditing={isEditing} onChange={handleChange} onEditClick={() => setIsEditing(true)} />
          <SystemDefaultsCard data={data} isEditing={isEditing} onChange={handleChange} />
        </div>

        {/* 右侧栏 */}
        <div className="flex flex-col gap-6 lg:col-span-1">
          <ContactInfoCard data={data} isEditing={isEditing} onChange={handleChange} />
          <BrandAssetsCard />
        </div>
      </div>
