...
const mockLogs = [
  {
    ...
    module: 'exchangeRate',
    moduleBadge: 'bg-amber-50 text-amber-700 dark:bg-amber-950 dark:text-amber-400',
    action: 'actionUpdate',
  ...
  },
  {
    ...
    user: '张三',
    initials: 'ZS',
    avatarColor: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-400',
    module: 'stockCheck',
    moduleBadge: 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-400',
    action: 'actionCreate',
      ...
      <div className="flex items-end gap-6 border-t border-slate-50 pt-6 dark:border-slate-800">
        <div className="max-w-md flex-1 space-y-1.5">
          ...
          <div className="flex items-center gap-2">
            <Input type="date" defaultValue="2026-03-20" className="h-10 bg-slate-50 dark:bg-slate-900/50" />
            <span className="text-slate-300">~</span>
            <Input type="date" defaultValue="2026-03-27" className="h-10 bg-slate-50 dark:bg-slate-900/50" />
          </div>
        </div>
...
function ChangeSummary({
  ...
  if (summary.type === 'text') {
    return <span className="text-slate-500 dark:text-slate-400">{summary.text}</span>
  }
  return (
    <span className="text-slate-500 dark:text-slate-400">
      {summary.field}: <span className="text-slate-300 line-through dark:text-slate-600">{summary.oldVal}</span> →{' '}
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950">
        ...
        <Table>
          ...
          <TableBody>
            {mockLogs.map((log, idx) => (
              ...
              >
                {/* 时间 */}
                <TableCell className="px-6 py-4 text-sm font-medium whitespace-nowrap text-slate-500 dark:text-slate-400">{log.time}</TableCell>
                {/* 用户（头像 + 名称） */}
                <TableCell className="px-6 py-4">
                  <div className="flex items-center gap-2">
                    <div className={cn('flex size-6 items-center justify-center rounded-full text-[10px] font-bold', log.avatarColor)}>
                      {log.initials}
                    </div>
                ...
                {/* 模块（彩色徽章） */}
                <TableCell className="px-6 py-4">
                  <span className={cn('rounded-full px-2 py-0.5 text-[11px] font-bold', log.moduleBadge)}>
                    {MODULE_LABELS[log.module] ?? log.module}
                  </span>
                </TableCell>
                {/* 操作（彩色文字） */}
                <TableCell className={cn('px-6 py-4 text-sm font-bold', log.actionColor)}>{t(log.action)}</TableCell>
                {/* 对象 */}
                <TableCell className="px-6 py-4 font-mono text-xs text-slate-600 dark:text-slate-400">{log.target}</TableCell>
                {/* 变更摘要 */}
      ...
      <div className="flex items-center justify-between border-t border-slate-100 bg-slate-50/50 px-6 py-4 dark:border-slate-800 dark:bg-slate-900/50">
        <p className="text-xs font-bold text-slate-400">{t('showingRange', { from: 1, to: 10, total: '1,248' })}</p>
        <div className="flex items-center gap-1">
          ...
          <button className="bg-primary flex size-8 items-center justify-center rounded text-xs font-bold text-white">1</button>
          {[2, 3].map(n => (
            <button
              key={n}
          ...
          <span className="px-2 text-xs text-slate-300 dark:text-slate-600">...</span>
