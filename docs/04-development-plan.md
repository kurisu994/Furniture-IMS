...
> **📊 进度**（截至 2026-04-23）：**2/14 项完成**。智能补货模块已完成：后端 7 个 IPC 命令（策略配置 + 建议计算 + 消耗趋势 + 一键生成采购单 + 忽略建议）+ 前端补货看板（建议列表 + 筛选 + 批量下单 + 消耗趋势图 + 策略配置弹窗）。首页看板 KPI 已接入实时补货数据。**剩余**：应付账款（4.1-4.2）、应收账款（4.3-4.4）、Recharts 封装（4.7）、采购/销售/库存报表（4.8-4.10）、首页看板接入真实数据（4.11）。
...
| M4：财务报表 + 补货      | 第 21-22 周     | 应收应付/**智能补货**/报表/**标准/实际毛利**/看板                                                                                                                        | 🔶 进行中（补货已完成） |
