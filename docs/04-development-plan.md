...
> **📊 进度**（截至 2026-04-17）：**12/15 项完成**（约 75%）。物料管理（Rust CRUD + 列表页 + 编辑弹窗）、分类管理（Rust 树结构 CRUD + 树形列表 + 编辑弹窗 + 拖拽排序）、供应商管理（Rust CRUD + 列表筛选分页 + 编辑弹窗 + 物料关联 Tab + 详情弹窗 + 删除）、客户管理（Rust CRUD 7 个 IPC 命令 + 列表筛选分页 + 编辑弹窗 + 详情弹窗 + 删除保护 + 编码自动生成 + 三语翻译）、系统设置基础（全部 8 个子页面）和 BOM 管理（Rust 10 个 IPC 命令 + 列表页 + 编辑页 + 明细弹窗 + 物料反查 + 需求计算 + 复制新版本 + 三语翻译）均已完成。**剩余**：通用组件抽取（2.13）、物料导入导出（2.16）。
...
> **📊 进度**（截至 2026-04-18）：**6/12 项完成**（约 50%）。采购单 Rust CRUD（7 个 IPC 命令）+ 列表页 + 编辑页（明细 inline 编辑 + 多币种 + 供应商物料快速带入）已完成。采购入库 Rust 层（3 个 IPC 命令，含库存操作基础模块 `inventory_ops.rs`）+ 前端（列表页 + 执行页）已完成。采购退货 Rust + 前端（3 个 IPC 命令 + 列表页 + 执行页）已完成。**剩余**：库存查询（3.13-3.14）、出入库流水（3.15）、期初库存导入（3.16）、操作日志（3.17-3.17a）。
...
> **📊 进度**（截至 2026-04-22）：**13/16 项完成**（约 81%）。销售单、销售出库、销售退货、定制单管理、库存盘点和库存调拨均已完成。生产工单模块（3.24-3.27）也已完成，涵盖后端 10 个 IPC 命令（CRUD+领料+退料+开始+完工）及前端列表/详情执行页。**剩余**：联调测试与缓冲（3.14，阶段整体回归）。
...
> **📊 进度**（截至 2026-04-24）：**10/14 项完成**。智能补货模块已完成：后端 7 个 IPC 命令 + 前端补货看板。首页看板 KPI 已接入实时补货数据。应付账款（4.1/4.1a/4.1b/4.2）已完成：修正 purchase.rs 入库/退货确认中 payables 记录生成逻辑，新增 finance.rs 后端 3 个 IPC 命令，前端列表页（KPI卡片+筛选+付款弹窗）。应收账款（4.3/4.3a/4.4）已完成：修正 sales.rs 中 receivables INSERT 字段名，新增后端 3 个 IPC 命令，前端列表页（KPI卡片+筛选+收款弹窗）。Recharts 图表组件已接入。库存报表（4.10）已完成：后端 4 个 IPC 命令 + 前端收发存/批次库龄/滞销预警 + Excel 导出。**剩余**：采购/销售报表（4.8-4.9）、首页看板接入真实数据（4.11）。
...
```
src-tauri/src/
├── lib.rs          # Tauri Builder — 日志 + 数据库初始化 + 管理员初始化 + IPC 注册（136 个命令）
├── main.rs         # 入口
...
| M3：销售+定制单+库存高级 | 第 17.5-18.5 周 | 销售/**单头仓库规则**/库存高级(盘点+调拨)/预警/**批次追溯**/**原材料预留 + 成品预留链路**/**期初库存导入**/**操作日志**/**定制单管理**/**生产工单(承接预留+领料+完工)**/ | 🔶 进行中（销售/库存/定制单/工单均已完成，待集成测试） |
| M4：财务报表 + 补货      | 第 21-22 周     | 应收应付/**智能补货**/报表/**标准/实际毛利**/看板                                                                                                                        | 🔶 进行中（财务+补货+库存报表已完成，剩余采购/销售报表+看板） |
