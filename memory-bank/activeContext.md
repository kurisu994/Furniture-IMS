...
项目处于 **功能完备、持续打磨** 阶段，当前版本 **v0.3.4**（2026-09-04 发布）。本月主要围绕"销售 → 生产"链路打通与权限精细化收敛：新增销售单下推生产、库管角色权限收紧，并修复一批上线前缺陷（Tauri 版本不匹配、记住我登录、看板权限报错等）。

## 本次会话（客户/供应商联系电话改为选填）

- **起因**：用户反馈客户表单的「联系电话」标签报红。定位到前端校验要求 `+区号` 开头的国际号码格式
  （`customer-helpers.ts` / `supplier-helpers.ts` 共用同一份正则），空值与国内号码写法都会被判失败；
  `Field` 的 `data-[invalid=true]:text-destructive` 使整行标签变红，而输入框边框不变，正是截图里的样子。
- **改动**：按用户要求只取消必填、保留格式校验。前后端一起放开——前端把「空则必填报错」改成「非空才校验格式」并去掉标签星号；
  后端 `customer.rs` / `supplier.rs` 的 `let Some(...) else { 联系电话不能为空 }` 改成与同文件 email 一致的
  `if let Some(...) { if !trim().is_empty() && !validate(...) }`。删除三语中不再引用的 `contactPhoneRequired` 文案。
- **为什么后端必须同改**：表单提交时空电话会转成 `null`，只改前端的话保存仍会被后端拦回「联系电话不能为空」。
- **验证**：`pnpm typecheck` 与 `cargo check` 均通过。
