...
- **库存报表模块（全栈）**：新增 `/reports/inventory` 页面与 4 个报表 IPC 命令，覆盖收发存汇总、批次库龄分析、滞销预警和库存趋势数据。
  - **收发存汇总**：支持日期范围、仓库、分类、物料类型和关键词筛选，展示期初/入库/出库/结存数量与 USD 基准金额 KPI。
  - **批次库龄分析**：基于启用批次追踪的库存批次计算在库天数，支持仓库/分类/库龄区间/关键词筛选，并提供库龄分布饼图。
  - **滞销预警**：按自定义未出库天数阈值筛选当前有库存物料，展示最后出库日、未出库天数和近 90 天月均出库量。
  - **Excel 导出**：当前报表 Tab 按生效筛选条件拉取全量数据并导出 `.xlsx` 文件。
  - 国际化：新增 `reports` 命名空间，覆盖中/越/英三语。
- **采购/销售报表模块（全栈）**：新增 `/reports/purchase` 与 `/reports/sales` 页面，并扩展 6 个报表 IPC 命令。
  - **采购报表**：按已确认入库单统计采购汇总趋势、供应商排行、物料采购明细，支持日期、供应商、仓库和关键词筛选。
  - **销售报表**：按已确认出库单统计销售汇总趋势、客户排行、产品销售明细，支持日期、客户、仓库和关键词筛选。
  - **共用报表体验**：KPI 卡片、Recharts 趋势图、业务表格分页、全量 Excel 导出和中/越/英三语文案复用同一套页面骨架。
