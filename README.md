...
src-tauri/                  # Rust 后端
  ...
  src/
    ...
    commands/
      ...
      reports.rs            # 报表中心命令：库存收发存/库龄/滞销/趋势（4 个命令）
...
- [x] 生产工单 — 工单 CRUD + 领料/退料 + 完工入库 + 状态流转（10 个 IPC 命令）
- [x] 智能补货 — 策略配置 + 建议计算 + 消耗趋势 + 一键生成采购单（7 个 IPC 命令）
- [x] 财务管理 — 应付/应收账款 + 收付款登记（6 个 IPC 命令）
- [x] 库存报表 — 收发存汇总 + 批次库龄 + 滞销预警 + Excel 导出（4 个 IPC 命令）
